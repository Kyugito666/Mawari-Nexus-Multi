...
INVITED_USERS_FILE = 'invited_users.txt'

# --- DAFTAR SECRET YANG TIDAK AKAN DITIMPA JIKA SUDAH ADA ---
PROTECTED_SECRETS = {
    "MAWARI_OWNER_ADDRESS",
    "MAWARI_BURNER_ADDRESS",
    "MAWARI_BURNER_PRIVATE_KEY",
...
def invite_collaborators():
    ...
    if not tokens_data or 'tokens' not in tokens_data:
        print("❌ FATAL: tokens.json tidak ditemukan atau formatnya salah."); return
...
def auto_set_secrets():
    print("\n--- Opsi 2: Auto Set Secrets (dengan Pengecekan) ---\n")
    secrets_to_set = load_json_file('secrets.json')
    if not secrets_to_set:
        print("❌ FATAL: secrets.json tidak ditemukan atau kosong."); return
    ...
    for index, token in enumerate(tokens):
        ...
        if not success:
            print(f"     - Fork tidak ditemukan. Membuat fork..."); run_command(f"gh repo fork {MAIN_ACCOUNT_USERNAME}/{BLUEPRINT_REPO_NAME} --clone=false --remote=false", env=env); time.sleep(5)
        else:
            print("     - Fork sudah ada.")

        # --- LOGIKA PENGECEKAN BARU ---
        print("   - Mengecek secrets yang sudah ada di repo...")
        success, existing_secrets_str = run_command(f'gh secret list --app codespaces --repo "{repo_full_name}" --json name -q ".[].name"')
        existing_secrets = set(existing_secrets_str.split()) if success and existing_secrets_str else set()
        if success:
            print(f"     - Ditemukan {len(existing_secrets)} secret: {', '.join(existing_secrets) if existing_secrets else 'Tidak ada.'}")
        # --- AKHIR LOGIKA PENGECEKAN BARU ---

        for name, value in secrets_to_set.items():
            # --- KONDISI BARU ---
            if name in PROTECTED_SECRETS and name in existing_secrets:
                print(f"   - ⏩ Melewati secret '{name}' karena sudah ada dan terlindungi.")
                continue
            # --- AKHIR KONDISI BARU ---
        ...
        try:
            ...
            for inv in invitations:
                ...
                if repo_name == target_repo:
                    print(f"     - Ditemukan undangan untuk {repo_name}. Menerima..."); accept_cmd = f"gh api --method PATCH /user/repository_invitations/{inv_id} --silent"
